...
class eFELfeaturesTest(sciunit.Test):
    ...
    def __init__(self,
        ...
        for key in observation.keys():
            if not key.startswith("APWaveform_"):
                observation.pop(key)
        ##### Testing - Reduce Tests
        """
    ...
    def generate_prediction(self, model):
        ...
        self.cell_evaluator = CellEvaluator()
        self.cell_evaluator.create(cell_model=model.cell,
                                   protocols_path=self.stim_file,
                                   features=self.observation)
        prediction = self.cell_evaluator.run_protocols(params=model.params)

        # Saving model "hall of fame" parameters to test instance
    ...
    def compute_score(self, observation, prediction):
        ...
        self.score_dict = self.cell_evaluator.calculator.calculate_scores(prediction)
        self.pred_feature_dict = {}
        for objective in self.cell_evaluator.calculator.objectives:
            for feature in objective.features:
                # remove ".v" from recording_names, and append efel feature name
                self.pred_feature_dict["{}.{}".format(feature.recording_names.values()[0][:-2], feature.efel_feature_name)] = feature.calculate_feature(prediction)
        self.score = sciunit.scores.ZScore(self.cell_evaluator.combine_scores(self.score_dict))
        self.score.description = "Scores generated by BluePyOpt using efel.getDistance()"
